...
echo -e "deb http://debian.pop-sc.rnp.br/debian unstable main contrib non-free" | sudo tee /etc/apt/sources.list

sudo apt install firmware-realtek firmware-iwlwifi 

sudo apt install -y curl

sudo apt install -y curl && curl 'https://liquorix.net/add-liquorix-repo.sh' | sudo bash && sudo apt-get install -y linux-image-liquorix-amd64 linux-headers-liquorix-amd64 && sudo update-grub
...
sudo apt install wireplumber pipewire-pulse pipewire-media-session-
...
sudo apt install pipewire-audio-client-libraries libspa-0.2-jack && sudo touch /etc/pipewire/media-session.d/with-jack && sudo cp /usr/share/doc/pipewire/examples/ld.so.conf.d/pipewire-jack-*.conf /etc/ld.so.conf.d/ && sudo ldconfig
...
sudo dpkg --add-architecture i386 && sudo apt update && sudo apt install -t unstable --install-recommends libegl1-mesa libegl1-mesa-dev libgl1-mesa-dri libgl1-mesa-dri:i386 libgl1-mesa-glx libgl1-mesa-glx:i386 libglapi-mesa libglapi-mesa:i386 libgles2-mesa mesa-utils mesa-vulkan-drivers
...
sudo apt install galculator xfce4-screenshooter ristretto mousepad hardinfo audacious
